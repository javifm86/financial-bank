<template>
  ...
  <div class="container mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="mt-4" v-else>
      ...
      <div class="grid gap-8 grid-cols-3 mb-8">
        <div class="bg-white overflow-hidden rounded-xl shadow">
          <div class="flex items-center p-8">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 mr-2 text-gray-400 fill-current" viewBox="0 0 640 512">
              <path
        ...
        <div class="bg-white overflow-hidden rounded-xl shadow">
          <div class="flex items-center p-8">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 mr-2 text-gray-400 fill-current" viewBox="0 0 512 512">
              <path
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="market in markets" :key="market.name">
                  <td class="px-5 py-5 text-sm text-left">{{ market.name }}</td>
                  <td class="px-5 py-5 text-sm text-right">{{ market.last }}</td>
                  <td class="px-5 py-5 text-sm text-right" :class="market.className">
                    {{ market.dif }}
                  </td>
                </tr>
              </tbody>
        ...
        <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
          ...
          <div class="inline-block min-w-full shadow sm:rounded-lg border-b border-gray-200 overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="elem in drawUpsDowns.ups" :key="elem.name">
                  <td class="px-5 py-5 text-sm text-left">{{ elem.name }}</td>
                  <td class="px-5 py-5 text-sm text-right">{{ elem.last }}</td>
                  <td class="px-5 py-5 text-sm text-right" :class="elem.className">
                    {{ elem.dif }}
                  </td>
                </tr>
                <tr v-for="elem in drawUpsDowns.downs" :key="elem.name">
                  <td class="px-5 py-5 text-sm text-left">{{ elem.name }}</td>
                  <td class="px-5 py-5 text-sm text-right">{{ elem.last }}</td>
                  <td class="px-5 py-5 text-sm text-right" :class="elem.className">
                    {{ elem.dif }}
      ...
      <div class="grid gap-8 grid-cols-3 mb-8 bg-white rounded-xl px-4 py-8 shadow">
        <article v-for="item in lastNews" :key="item.id">
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      drawUpsDowns: {} as {
        ups: { name: string; last: string; dif: string; className: string }[];
        downs: { name: string; last: string; dif: string; className: string }[];
      },
      markets: [] as { name: string; last: string; dif: string; className: string }[],
  ...
  methods: {
    initUserInfo(data: GetInfo) {
      ...
      this.wealth = this.formatCurrency(data.accounts[0].wealth, data.accounts[0].currency);
      this.operatingLimit = this.formatCurrency(data.accounts[0].operatingLimit, data.accounts[0].currency);
      this.balance = this.formatCurrency(data.accounts[0].balance, data.accounts[0].currency);
    },
    ...
    formatMarket(elem: Market) {
      return {
        name: elem.name,
        last: this.formatNumber(elem.last),
        dif: this.formatPercent(elem.dif),
    ...
    formatNews(elem: News) {
      return {
        ...
        date: this.formatDate(elem.date)
      };
    },
    ...
    homeService
      ...
      .then((response) => {
        ...
        if (this.upDowns == null) {
          ...
          this.drawUpsDowns.ups = this.upDowns.ups.map(this.formatMarket);
          this.drawUpsDowns.downs = this.upDowns.downs.map(this.formatMarket);
        }
      })
      .catch((e) => {
        console.warn(e);
  ...
  mounted() {
    ...
    console.warn(this.drawUpsDowns);
  }
});
</script>
